...
public class ProductServiceImpl implements ProductService {
    ...
    public Product save(Product product) {
        ...
        if(name == null || name.trim().isEmpty() || name.length() <3) {
            throw new ProductSaveException("Product name should be at least 3 character long");
        }

        if(product.getPrice() <= 0){
            throw new ProductSaveException("Product price cannot by negative and zero");
        }
    ...
    public void update(Product product) {
        ...
        String name = product.getName();
        if(name == null || name.trim().isEmpty() || name.length() <3) {
            throw new ProductUpdateException("Product name should be at least 3 character long");
        }

        if(product.getPrice() <= 0){
            throw new ProductUpdateException("Product price cannot by negative and zero");
        }
    ...
    public void deleteByName(String name) {
        ...
        if(product == null || !product.isActive()){
            throw new ProductNotFoundException("Product with id" + name + "not found");
        }
